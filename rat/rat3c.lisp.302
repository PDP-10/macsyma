...
(DEFUN PGCDU1 (U V PQUO*)
  (LOOP WITH INVV = (PAINVMOD (PT-LC V))
	FOR K #-Multics FIXNUM = (- (PT-LE U) (PT-LE V))
	UNTIL (MINUSP K)
	FOR Q* = (PTIMES INVV (PT-LC U))
	IF PQUO* DO (SETQ QUO (NCONC QUO (LIST K Q*)))
	WHEN (PTZEROP (SETQ U (PQUOTIENT2 (PT-RED U) (PT-RED V))))
	     RETURN (PTZERO)
	FINALLY (RETURN U)))

(DECLARE (UNSPECIAL K Q* QUO)
	 (NOTYPE K))
...
(DEFUN BIGPRIMES ()
  (SETQ BIGPRIMES ())
  (DO ((I 0 (1+ I))
       (P ;; *:MAX-FIXNUM can't be READ on LISPM
	(LSH -1 -1)
	(NEWPRIME P)))
      ((= I 20.))))
...
#+LISPM (DEFMVAR BIGPRIMES NIL)
#+LISPM (DO ((I 0 (1+ I))				;GENERATES 20 LARGEST
...
(DEFMFUN $PRIMEP (P)
 (IF (NOT (FIXP P)) (MERROR "Argument to PRIMEP must be an integer:~%~M" P))
